---
title: "Comparing Carbon Isotopes in Open and Closed Systemss"
...
# System Overview: Carbonate Speciation
  We are going to follow $\delta^{13}C$ as $CO_{2(g)}$ dissolves in water and carboante species form at equilibrium. Below is the cascade of reactions that describe the speciation of carbonate in aqueous solution. 
...
### Question 1
Q: Before we attempt to model an open system at equilibrium: What parameters will we need to fix and which should we let vary?
  A:  Fixed... temperature, pCO2
      Variable... pH
...
### Question 2 
Q: Plot the carboante speciation for the above open system at equilibrium.  
  
```{r plot open speciation}
#A: Using a log scale on the y-axis, we can recreate the above plot to double check our work for the open, equilibrium system. 
p_1 <- isotopes_open %>% 
...
Q: What do you observe about how the isotopic values of the carboante species vary with pH? 
  A: The isotopic composition of each species is independent from pH. The isotopic value for each species is only determined by the fractionation factor between each species which is ultimately pinned to the value of $\delta^{13}C_{pCO_2}$ , which is set by temperature, not pH. Only the $\delta^{13}C_{DIC}$ varies with pH because it is a sum of all of the isotope compositions in proportion to how much of each species exists at every pH step. 
...
Q: What environment or natural setting does an open system model well?
  A: Shallow aqueous environments, such as mixed lakes or wetlands, or soil environments that exchange with the atmosphere through diffusion. Open systems do assume that the solution is in constant contact with a large reservior or $CO_2$.
...
### Question 5
Q: Before we attempt to model an open system at equilibrium: What parameters will we need to fix and which should we let vary?
  A:  Fixed... temperature, [DIC], $\delta_{DIC}$
      Variable... pH
      Note: this system does not have a pCO2 variable, but its a closed system that is not in contact with an atmosphere. There is a set concentration and isotopic value for DIC, but those quantities are finite. 
...
### Question 6
Q: What do you observe about how the isotopic values of the carboante species vary with pH? 
  A: Now for a closed system at equilibrium, the isotopic composition of each species is dependent on pH. Each species is separated by a known fraction factor, but the $\delta^{13}C$ varies with pH because the the concentration of each species changes with pH. The $\delta^{13}C_{DIC}$, however, is now constant because the isotopic values and fraction of each species must add back to the original $\delta^{13}C_{DIC}$ composition. 
  
### Question 7
Q: What environment or natural setting does a closed system model well?
  A: Environment that does not have exchange with atmosphere... parsel of water starting at ocean surface and then decending away from the surface so no more exchange with the atmosphere can occur. Others? caves? soil water? interior carbonate reaction within biogenically mediated carbonate precipitation. 
