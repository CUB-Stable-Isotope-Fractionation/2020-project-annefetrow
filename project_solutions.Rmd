...
Q: Before we model an open system at equilibrium: What parameters will we need to fix and which should we let vary?
  A:  
    - Fixed... temperature, pCO2, $\delta^{13}C_{pCO_2}$. 
    - Variable... pH
...
p_2 <- isotopes_open %>% 
  pivot_longer(cols = c(d_CO2_g, d_CO2_aq, d_HCO3_aq, d_CO3_aq, d_calcite_min, d_DIC)) %>% 
    ggplot() +
      ...
      theme_bw() + theme(text = element_text(size = 13)) +
      scale_x_continuous(expand = c(0, 0)) +
      labs(y = "d 13 C", color = NULL, title = "Carbon Isotopes - Open System at Equilibrium")
p_2
```
...
Q: Before we model an open system at equilibrium: What parameters will we need to fix and which should we let vary?
  A: 
    - Fixed... temperature, [DIC], $\delta_{DIC}$
    - Variable... pH
...
p_4 <- isotopes_closed %>% 
  pivot_longer(cols = c(d_DIC, d_bicarb, d_CA, d_carb, d_calcite_min)) %>% 
    ggplot() +
      ...
      theme_bw() + theme(text = element_text(size = 13)) +
      scale_x_continuous(expand = c(0, 0)) +
      labs(y = "d 13 C", color = NULL, title = "Carbon Isotopes - Closed System at Equilibrium")
p_4
```
### Question 6
Q: What do you observe about how the isotopic values of the carboante species vary with pH? 
  A: Now for a closed system at equilibrium, the isotopic composition of each species is dependent on pH. Each species is separated by a known fraction factor, but the $\delta^{13}C$ varies because the the concentration of each species changes with pH. The $\delta^{13}C_{DIC}$, however, is now constant because the (isotopic values * fraction) of each species must add back to the original $\delta^{13}C_{DIC}$ composition. 
