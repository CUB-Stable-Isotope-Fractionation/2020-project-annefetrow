...
# Exercise 1: Pre-Industrial pCO2, starting conditions
```{r, fig.width = 8, fig.height = 6}
O_isopath <- O_isopath %>% 
                  set_parameters(
                    tibble(
                    # define scenarios to model
                      scenario = c("pre-indust"),  
...
## Exercise 3: vary temperature
  According to Henry's Law, the temperature of a system affects the solubility of $CO_2$ in aqueous solution. 
```{r vary temp}
solubility <- tibble(
...
# set range of pCO2 levels for 2 scenarios. 
    #1) pre-industrial modern pCO2 levels: 280 ppm (2.8E-4 atm, 0.284 mbar)
...
df_pH_vs_pCO2 <-
  ...
  mutate(
    pH = 10^-7, #calculate_open_system_pH(pCO2, temp = temps),
